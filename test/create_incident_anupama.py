...
class create_incident:
    ...
    def __init__(self, account_num, incident_id):
        ...
        # Initialize document
        self.initialize_mongo_doc()

        # Load customer details
        status = self.read_customer_details()
        if status != "success":
            print("Failed to load customer details. Exiting.")
            sys.exit(1)

        # Load payment data
        status = self.get_payment_data()
        if status != "success":
            print("Failed to load payment details. Exiting.")
            sys.exit(1)

        # Format and show final JSON
        formatted_json = self.format_json_object()
...
if __name__ == "__main__":
    create_incident(account_num="0000003746", incident_id="67890")
